...
ckpts = sorted(glob.glob("checkpoints/ddpm/ddpm_*.pth"), key=lambda p: int(re.search(r"ddpm_(\d+)", p).group(1)))
unet = UNet().to(device)
...
epoch = re.search(r"ddpm_(\d+)", ckpts[-1]).group(1)
out_path = f"output/ddpm/ddim_stride{stride}_eta{eta}_ep{epoch}.png"
